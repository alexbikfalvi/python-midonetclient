...
# build directory.
gen_deb() {
    log "Generating version-dependent files"
    VERSION="$1"
...
package_deb() {
    ...
    if [ "$1" == "" ]
    then
        error "DEBIAN build requires a release number"
        exit -1
    fi

    gen_deb $version
 
    log "Generating DEBIAN packages"
    ...
    log "DEBIAN packages ready at $destdir"
}
...
package_rhel() {
    ...
    log "Generating RHEL packages for version: $rpm_ver"
    ...
    log "RHEL packages are ready at $BUILD_DIR/rhel"
}
...
log "Welcome to Midokura's python-midonetclient packaging experience"
